...
def nonl(obl):
    ...
    for s in target:
        ...
        return (
            ...
            - datetime.datetime.strptime(d["dates"][-1], "%d.%m.%Y")
        ).days > 3
...
def calc(site, day=True, cost=None):
    ...
    try:
        ...
        if resp.status_code != 200:
            return {}
        obl = BeautifulSoup(resp.text, "lxml")
    except:
        return {}
    ...
    if name in kval or name in no_exist:
        return {}
    try:
        nex = datetime.datetime.strptime(extr(obl, "Дата след. выплаты"), "%d-%m-%Y")
    except ValueError:
        no_date.append(name)
        return {}
    except Exception:
        if day:
            ...
            nex = datetime.datetime.strptime(
                obl.findAll("table")[1].findAll("td")[1].string, "%d-%m-%Y"
            )
    ...
    if len(tab) > 1:
        ...
        for coup in tab.findAll("tr")[1:]:
            n = datetime.datetime.strptime(coup.contents[3].string, "%d-%m-%Y").date()
...
def parse(table, site, day=True):
    ...
    for i in range(2, last_page + 2):
        ...
        try:
            if day:
                lst = BeautifulSoup(requests.get(site + f"/page{i}/").text, "lxml")
            else:
                lst = BeautifulSoup(
                    requests.get(site + f"/order_by_val_to_day/desc/page{i}/").text,
                    "lxml",
                )
        except:
            return table
